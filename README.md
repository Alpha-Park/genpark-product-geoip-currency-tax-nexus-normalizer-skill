# genpark-product-geoip-currency-tax-nexus-normalizer-skill
...
**genpark-product-geoip-currency-tax-nexus-normalizer-skill** is a deterministic, zero-dependency Python skill engineered for autonomous AI workflows, multi-agent orchestration, and production deployments.

> **Executive Capability**: Cross-border Geo-IP currency converter & dynamic VAT/tax nexus calculator (Stripe Tax)
...
result = client.normalize_currency_and_tax()
print(result)
```
...
```json
{
  "mcpServers": {
    "genpark-product-geoip-currency-tax-nexus-normalizer-skill": {
      "command": "python",
      "args": ["/path/to/genpark-product-geoip-currency-tax-nexus-normalizer-skill/mcp_server.py"]
    }
  }
}
```
